...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!SLUGS_IR.includes(slug)) return {}
  const pagina = gerarPaginaIR(slug)
  return {
    ...
    openGraph: {
      ...
      description: pagina.metaDesc,
      url: `https://calculadoravirtual.com/ir/${slug}`,
      siteName: 'Calculadora Virtual',
      locale: 'pt_BR',
      type: 'article',
      publishedTime: pagina.publishedAt,
    },
    alternates: { canonical: `/ir/${slug}` },
  }
}

// Slugs relacionados ao tema atual
function slugsRelacionados(slug: string): string[] {
  const todos = SLUGS_IR.filter(s => s !== slug)
  const relacionados: string[] = []
  // tenta pegar slugs com palavras em comum
...
export default async function PaginaIR({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  if (!SLUGS_IR.includes(slug)) notFound()
    ...
    <>
      ...
      <div className="container" style={{ paddingTop: 28, paddingBottom: 60 }}>
        ...
        >
          ...
          <span>›</span>
          <Link href="/ir" style={{ color: 'var(--brand)', textDecoration: 'none' }}>Imposto de Renda</Link>
          <span>›</span>
          <span style={{ color: 'var(--dim)' }}>{pagina.titulo.slice(0, 40)}{pagina.titulo.length > 40 ? '…' : ''}</span>
        </nav>
                ...
                <div style={{
                  ...
                  <div>
                    ...
                    <div style={{ color: 'var(--muted)', fontSize: '0.88rem' }}>
                      Use nossa calculadora de IRPF 2025 — atualizada com a tabela vigente.
                    </div>
                  </div>
                  <Link
                    href="/ferramentas/calculadora-irpf"
                    style={{
                      background: 'var(--brand)',
                      color: '#fff',
                      textDecoration: 'none',
          ...
          <div className="sidebar-col" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div className="card" style={{ padding: 16 }}>
              ...
              <div style={{ fontSize: '0.8rem' }}>
                {TABELA_IRPF_2025.map((f, i) => (
                  <div key={i} style={{
                    display: 'flex',
                    justifyContent: 'space-between',
                    padding: '5px 0',
                    borderBottom: i < TABELA_IRPF_2025.length - 1 ? '1px solid var(--line)' : 'none',
                    color: 'var(--muted)',
                  }}>
                    <span>{f.aliquota === 0 ? 'Isento' : `${f.aliquota}%`}</span>
                    <span style={{ color: 'var(--dim)' }}>
                      {f.ate ? `até R$ ${f.ate.toLocaleString('pt-BR')}` : 'acima'}
                    </span>
                  </div>
                ))}
              </div>
              <Link
                href="/ir/tabela-irpf-2025"
            ...
            <div className="card" style={{ padding: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <Link href="/ferramentas/calculadora-irpf" style={{ fontSize: '0.85rem', color: 'var(--brand)', textDecoration: 'none' }}>🧮 Calculadora IRPF</Link>
