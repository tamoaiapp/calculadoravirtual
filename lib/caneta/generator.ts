...
export function gerarPaginaMedicamento(med: MedicamentoCaneta): PaginaCaneta {
  ...
  const secoes: SecaoPagina[] = [
    ...
    {
      h2: `O que compromete o tratamento com ${med.nome} — erros documentados`,
      conteudo: `Alguns erros aparecem com tanta frequência que os próprios estudos de adesão ao tratamento os documentam. O mais crítico: abandonar o tratamento nas primeiras 4–8 semanas por causa dos efeitos colaterais — antes da fase de adaptação terminar e antes de qualquer resultado aparecer na balança. Os outros erros que médicos e pacientes relatam com mais frequência:`,
      lista: [
  ...
  return {
    ...
    intro: `${med.nome} entrou no vocabulário médico brasileiro de vez. A prescrição de ${med.principioAtivo} cresceu de tal forma que, em 2023, o Brasil enfrentou escassez nas farmácias — não por falta de demanda, mas porque a demanda superou toda projeção da indústria. Os números dos ensaios clínicos explicam o fenômeno: perdas de peso que medicamentos anteriores simplesmente não conseguiam.\n\nMas popularidade tem um custo em desinformação. Circulam versões manipuladas ilegais, protocolos de dose tirados de grupos de WhatsApp, e promessas de resultado que ignoram o que os próprios estudos dizem sobre variação individual. O que você vai encontrar aqui é diferente: os dados reais dos ensaios — incluindo as limitações — e o que a prática clínica mostra além dos números.\n\n⚠️ ${med.nome} exige prescrição médica. As informações a seguir são educativas e não substituem avaliação clínica individual.`,
    secoes,
    faq,
    conclusao: `Os estudos ${med.slug === 'mounjaro' || med.slug === 'zepbound' ? 'SURMOUNT-1 e SURPASS-2' : 'STEP-1 e SUSTAIN-6'} produziram resultados que nenhum medicamento para obesidade havia alcançado antes — e não foi só em termos de peso. Houve melhora de pressão arterial, colesterol, HbA1c e risco cardiovascular em populações que carregavam essas comorbidades há anos. Esses números são reais.\n\nMas há um porém que os estudos também documentaram: quem parou o medicamento sem mudar os hábitos recuperou dois terços do peso em um ano (dados do STEP-1 Extension). Isso não desqualifica o tratamento — mostra que ele funciona como um instrumento, não como uma solução autônoma. O peso volta porque a obesidade é uma doença crônica, e não porque o medicamento falhou.\n\nA decisão de usar ${med.nome} é clínica. Cerca de 15% dos pacientes têm resposta abaixo da esperada, e o custo ainda é uma barreira real para muita gente. Converse com um endocrinologista — com os dados dos estudos na mão, a conversa fica mais produtiva.`,
    tipo: 'medicamento',
    aviso: AVISO_MEDICO,
  }
}
...
export function gerarPaginaEfeito(efeito: string, medicamentosAfetados: string[]): PaginaCaneta {
  ...
  return {
    ...
    secoes,
    faq,
    conclusao: `${nomeFormatado} acontece porque o medicamento está funcionando — essa é a parte contraintuitiva. O mesmo mecanismo que retarda o esvaziamento gástrico e suprime o apetite é o que causa desconforto nas primeiras semanas. À medida que os receptores GLP-1 se adaptam, os sintomas diminuem na maioria dos pacientes. No STEP-1, a frequência de náusea caiu de 44% nas semanas iniciais para menos de 10% após a semana 20.\n\nO erro mais comum: desistir exatamente nessa janela de 4 a 8 semanas, quando o corpo ainda está se ajustando e a balança ainda não registrou resultado. Fale com o médico antes de interromper — existem estratégias de manejo (ajuste de dose, mudança de horário, alimentação adaptada) que funcionam sem precisar parar o tratamento.`,
    tipo: 'efeito',
    aviso: AVISO_MEDICO,
  }
}
...
export function gerarPaginaComparativo(slugMed1: string, slugMed2: string): PaginaCaneta {
  ...
  const secoes: SecaoPagina[] = [
    ...
    },
    {
      ...
      conteudo: `Ambos têm perfil de efeitos colaterais dominado por sintomas gastrointestinais — e nenhum deles "ganha" em tolerabilidade de forma absoluta. A dose muda o jogo: doses mais altas causam mais efeitos, independente do medicamento. O que os estudos clínicos registraram (dados das bulas aprovadas e dos ensaios pivotais):`,
      tabela: {
        cab: ['Efeito colateral', med1.nome, med2.nome],
        linhas: [
    ...
    {
      h2: 'Preço no Brasil: qual oferece melhor custo por resultado?',
      conteudo: `Comparar preço entre ${med1.nome} (R$ ${med1.precoMedio.toLocaleString('pt-BR')}/mês em média) e ${med2.nome} (R$ ${med2.precoMedio.toLocaleString('pt-BR')}/mês em média) exige considerar o resultado esperado, não só o valor mensal. Um medicamento mais caro que produz perda de 20% do peso pode ser economicamente mais eficiente do que um mais barato que produz 10% — especialmente se a diferença implica reduzir comorbidades como hipertensão, apneia e diabetes tipo 2, que têm custos próprios.\n\nO lado prático que os cálculos não mostram: os dois são de dose progressiva, então o custo do primeiro mês (dose inicial) é menor que o custo de manutenção (dose plena). Considere o custo anual — não só o mensal — no planejamento financeiro. Nenhum dos dois tem genérico aprovado no Brasil, e farmácias de manipulação não podem manipular semaglutida ou tirzepatida legalmente.`,
    },
  ]
  ...
  const faq: { q: string; a: string }[] = [
    ...
    {
      q: `Existe alguma forma de reduzir o custo de ${med1.nome} ou ${med2.nome}?`,
      a: `Algumas estratégias práticas: 1) Programas de fidelidade de farmácias de rede (Ultrafarma, Panvel, Drogasil, Pague Menos) costumam ter descontos de 5–15% para clientes cadastrados. 2) A CMED da ANVISA fixa o preço máximo — qualquer valor acima é ilegal, e vale comparar antes de comprar. 3) O IRPF permite dedução de despesas médicas com receita, incluindo medicamentos de prescrição. 4) Programas de assistência ao paciente dos próprios fabricantes (Novo Nordisk e Eli Lilly têm programas ativos) podem oferecer descontos para pacientes de baixa renda. Pergunte diretamente à farmácia ou ao médico — esses programas não são amplamente divulgados.`,
    },
  ]
  ...
  return {
    ...
    intro: `"Qual é melhor — ${med1.nome} ou ${med2.nome}?" A pergunta é simples. A resposta, não. ${mesmoPrincipio ? `Os dois contêm ${med1.principioAtivo}, então a diferença não está na molécula — está na dose e no que essa diferença de dose significa para resultado e efeitos colaterais.` : `São moléculas diferentes (${med1.principioAtivo} vs. ${med2.principioAtivo}), com mecanismos distintos e dados clínicos que não deveriam ser comparados como se fossem equivalentes.`}\n\nO problema de comparar medicamentos com base em opiniões de fóruns e grupos de WhatsApp: cada relato reflete um metabolismo, uma dose e um contexto clínico específico. Os dados dos ensaios clínicos — STEP-1, SURMOUNT-1, SUSTAIN-6 — existem justamente para ir além do anedótico. É com esses números que essa comparação trabalha.`,
    secoes,
    faq,
    conclusao: `${mesmoPrincipio ? `${med1.nome} e ${med2.nome} são o mesmo princípio ativo em doses diferentes — e dose importa. Mais dose significa mais resultado, mas também mais efeito colateral. A pergunta clínica real não é "qual desses", mas "que dose você tolera e por quanto tempo?"` : `${med1.nome} e ${med2.nome} resolvem o mesmo problema por caminhos diferentes. Para a maioria dos pacientes, a escolha é guiada por tolerabilidade e custo — não por superioridade absoluta de um sobre o outro.`}\n\nO que os estudos mostram que realmente muda o resultado: adesão ao tratamento ao longo do tempo, qualidade da alimentação durante o uso, e acompanhamento médico regular. Medicamento certo + dose errada + nenhuma mudança de hábito produz resultado mediano. Leve os dados desta comparação para o seu endocrinologista — e pergunte qual faz mais sentido para o seu histórico específico.`,
    tipo: 'comparativo',
    aviso: AVISO_MEDICO,
  }
}
...
export function gerarPaginaGuia(slug: string): PaginaCaneta {
  const guias: Record<string, PaginaCaneta> = {
    'como-aplicar-ozempic': {
      ...
      intro: 'A maioria dos erros com o Ozempic não acontece na dose nem no horário — acontece na técnica de aplicação. Uma agulha reusada, o tempo de 6 segundos ignorado, o local errado: cada detalhe influencia tanto a absorção da semaglutida quanto o desconforto sentido. Este guia cobre cada etapa da aplicação, do que separar na bancada até o descarte correto da agulha.',
      secoes: [
      ...
      conclusao: 'A técnica de aplicação melhora com a prática — a maioria das pessoas acha a segunda aplicação muito mais fácil que a primeira. O ponto que mais compromete o resultado: a agulha reusada. Com reusos repetidos, a ponta microscópica da agulha dobra e causa microtraumas que levam à lipodistrofia — o endurecimento da pele que prejudica a absorção da semaglutida. Uma caixa de agulhas avulsas custa poucos reais e evita esse problema completamente. Se tiver dúvida na técnica, peça demonstração na farmácia ou na próxima consulta com a enfermagem.',
      tipo: 'guia',
      aviso: AVISO_MEDICO,
    },
    'dieta-com-ozempic': {
      ...
      intro: 'Uma das surpresas de quem começa o Ozempic: o apetite cai, mas os resultados na balança dependem do que você coloca no prato. Não porque o medicamento falha — mas porque comer os alimentos errados (gordurosos, ultraprocessados, porções grandes) piora a náusea e sabota o déficit calórico que produz a perda de peso. No estudo STEP-1, quem seguiu protocolo alimentar perdeu em média 14,9% do peso; estimativas de uso "livre" no mundo real ficam em torno de 8–10%. A diferença é real.',
      secoes: [
      ...
      conclusao: 'O Ozempic muda a relação com a comida — não elimina a necessidade de escolher bem. A redução de apetite criada pela semaglutida é uma janela: use-a para reformatar hábitos, não apenas para comer menos do mesmo. Quem aproveita esse período para consolidar padrões alimentares melhores tem mais chances de manter os resultados quando (e se) o medicamento for interrompido. Um nutricionista com experiência em tratamento com GLP-1 faz diferença real no resultado final.',
      tipo: 'guia',
      aviso: AVISO_MEDICO,
    },
  }
  ...
  const guiaGenerico: PaginaCaneta = {
    ...
    intro: `${slug.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())} é um dos temas que mais gera dúvidas entre pessoas que usam ou estão considerando canetas emagrecedoras como Ozempic (semaglutida), Wegovy (semaglutida 2,4mg) ou Mounjaro (tirzepatida). As informações a seguir têm base nos grandes ensaios clínicos publicados — STEP, SUSTAIN, SURMOUNT e SCALE — e têm caráter exclusivamente educativo. Não substituem avaliação médica individual.`,
    secoes: [
        { h2: 'O que as canetas emagrecedoras têm em comum — e o que as separa', conteudo: 'Ozempic, Wegovy, Mounjaro, Zepbound e Saxenda pertencem à mesma família de medicamentos (análogos de GLP-1 ou GIP/GLP-1), mas não são equivalentes. A semaglutida do Ozempic 1mg e a do Wegovy 2,4mg são a mesma molécula em doses diferentes — e a diferença de dose importa: o STEP-1 mostrou perda de 14,9% com 2,4mg versus 6% com 1mg. Já a tirzepatida (Mounjaro/Zepbound) é uma molécula distinta, com ação em dois receptores (GLP-1 e GIP), e tem os maiores resultados publicados: até 22,5% de perda no SURMOUNT-1. Saxenda (liraglutida) é a mais antiga da classe, com aplicação diária e resultados menores — mas opção real para quem não tolera as versões semanais.' },
      { h2: 'Como os estudos clínicos são conduzidos — e por que os números importam', conteudo: 'As informações desta página vêm dos grandes ensaios clínicos publicados em revistas revisadas por pares. SUSTAIN-6 e STEP-1 (semaglutida, 2016 e 2021, respectivamente, no New England Journal of Medicine), SCALE (liraglutida, 2015, no mesmo periódico) e SURMOUNT-1 (tirzepatida, 2022, também no NEJM). Esses estudos são duplo-cegos e randomizados — o padrão mais alto de evidência em medicina. O que eles têm em comum: todos incluíram protocolo alimentar e de exercício, o que significa que os percentuais de perda de peso refletem o medicamento combinado com mudança de hábito, não o medicamento sozinho.' },
      { h2: 'Por que o médico não é opcional nesse tratamento', conteudo: 'Análogos de GLP-1 como Ozempic e Wegovy não são suplementos — são medicamentos com interações medicamentosas reais, contraindicações absolutas e protocolo de progressão de dose que precisa ser monitorado. Quem compra sem receita ou segue protocolos de grupos de WhatsApp tem taxas de abandono por efeitos colaterais muito maiores. O acompanhamento médico muda o resultado. No estudo de extensão STEP-5 (2 anos), pacientes com seguimento regular tiveram perda de peso 40% maior do que a média dos estudos de curto prazo.', lista: ['Consulta a cada 4–8 semanas nos primeiros 6 meses (retorno mais frequente que o habitual)', 'Exames laboratoriais antes de iniciar e a cada 3–6 meses: lipase, tireoide, HbA1c', 'Avaliação do peso, circunferência abdominal e pressão — não apenas a balança', 'Revisão de todos os medicamentos em uso — vários interagem com análogos de GLP-1'] },
    ],
    faq: [
    ...
    conclusao: 'Canetas emagrecedoras mudaram o que é possível em termos de perda de peso farmacológica — mas o tratamento bem-sucedido exige mais do que a injeção semanal. A combinação de medicamento, alimentação ajustada, exercício e acompanhamento médico regular é o que os próprios estudos clínicos replicaram para chegar nos números que tornaram esses medicamentos famosos. Leve suas dúvidas específicas para um endocrinologista — é a especialidade mais adequada para esse tipo de tratamento.',
    tipo: 'guia',
    aviso: AVISO_MEDICO,
  }
...
export function gerarPaginaCalculadora(slug: string): PaginaCaneta {
  const calcPage: PaginaCaneta = {
    ...
    intro: 'Quanto peso você vai perder com Ozempic, Wegovy ou Mounjaro? A resposta honesta é: depende. Os estudos STEP-1 (semaglutida 2,4mg) e SURMOUNT-1 (tirzepatida 15mg) mostram médias de 14,9% e 20,9% do peso corporal, respectivamente — mas a variação individual é enorme. Esta calculadora traduz esses percentuais para quilogramas reais com base no seu peso, medicamento e tempo de tratamento. Use o resultado como ponto de partida para a conversa com seu médico, não como promessa.',
    secoes: [
